...
Questa esercitazione ti introdurrà all'installazione e all'esecuzione dei primi comandi Docker. Assicurati di avere un ambiente Alpine Linux pronto ( Che tovate già pronto nella dir machine - usate alpineNAT ).
* Assicurati di soddisfare i **requisiti di sistema**: Alpine Linux 3.13+, Kernel 3.10+, accesso root/sudo, connessione internet.  
  (Tranquilli, questi requisiti sono quasi sempre già soddisfatti.)
...
4.  **Gestione del Ciclo di Vita dei Container**:
    ...
    *   Sperimenta con questi comandi utilizzando i container creati nei passaggi precedenti. Ricorda che i container sono effimeri (Nell'ultimo livello) e le modifiche non salvate in volumi vengono perse all'eliminazione.
...
**Crea un file chiamato `Dockerfile` nella stessa directory con un contenuto simile a questo:**
        ```dockerfile
        FROM node:18-alpine
        WORKDIR /app
        COPY package*.json ./
    ...
    *   Spiegazione delle istruzioni:
        ...
        *   `CMD [ "npm", "start" ]`: Specifica il comando per avviare l'applicazione (assumendo che tu abbia uno script `start` in `package.json`).
...
3.  **Costruzione dell'Immagine Docker**:
    ...
    *   Verifica che l'immagine sia stata creata con `docker images`.
...
4.  **Esecuzione di un Container dall'Immagine**:
    ...
    *   Verifica che l'applicazione funzioni aprendo un browser su `http://localhost:8080`.
    *   Visualizza i log con `docker logs <ID_DEL_CONTAINER>`.

**Esercitazione 3: Networking e Persistenza Dati con Docker**
Ecco una versione più ricca di contesto teorico e spiegazioni per ciascuna parte dell’esercitazione:
